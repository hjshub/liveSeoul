typeof navigator === "object" && (function (global, factory) {
      ...
      value: function media() {
        ...
        if (player.supported.ui && player.config.clickToPlay && !player.isAudio) {
          ...
          on.call(player, elements.container, 'click', function (event) {
            ...
            /* 모바일 화면에서 화면 클릭 했을 때 정지가 안되는부분 수정.
  ...
  var parseVtt = function parseVtt(vttDataString, vttUrl) {
    var processedList = [];
    ...
    frames.forEach(function (frame) {
      ...
      lines.forEach(function (line) {
        if (!is$1.number(result.startTime)) {
          ...
          result.text = vttUrl + _lineSplit[0];
    ...
    _createClass(PreviewThumbnails, [{
      ...
      value: function getThumbnail(url) {
        ...
        if(url != null && url != "" && url.indexOf("http") > -1) {
        	vttUrl = url.substring(0, url.indexOf("/upload"));
        }

        return new Promise(function (resolve) {
          fetch(url).then(function (response) {
            var thumbnail = {
              frames: parseVtt(response, vttUrl),
              height: null,
              urlPrefix: ''
  ...
  function () {
    function Plyr(target, options) {
      ...
      // Player Unique Key Setting
      this.storage.key = plyrKey;
